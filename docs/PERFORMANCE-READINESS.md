...
- Pembaruan setelah pengukuran di atas: fence revisi sekarang per collection yang dibaca/ditulis, bukan global. Transaksi pada collection terpisah tidak saling retry; collection yang tumpang tindih tetap berbagi fence, termasuk pembacaan `users`. Batas batch 2.000 mencakup operasi penyimpanan dan seluruh fence. Terapkan perubahan schema `(scope, sequence)` saat seluruh writer lama berhenti; lihat panduan deployment. Pengukuran payload di atas tidak mengukur throughput perubahan fence ini.
