...
## Konfigurasi SMTP eksternal

Pengiriman eksternal melalui Postmark sudah diuji dari instance lokal. Uji aktivasi pada 14 September 2026 menghasilkan status `sent` setelah satu percobaan tanpa error SMTP; status ini tidak menjamin pesan masuk ke inbox penerima. Petunjuk konfigurasi tersedia pada `.env.example` dan validasi mailer di `db-schema/pb_hooks/accounts.js`.
