...
- Rate limit: login 10 percobaan per kombinasi IP/identitas dalam 15 menit (akun QA lokal dikecualikan); permintaan email 5 per alamat/jam dan 20 per IP/jam; pemeriksaan/konfirmasi token 60 per IP/15 menit. Permintaan email yang tidak terdaftar memberikan respons umum. Di arsitektur proxy saat ini IP yang terlihat PocketBase dapat merupakan IP server SvelteKit, sehingga limit IP sengaja konservatif.
