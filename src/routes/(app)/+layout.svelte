...
{#if authorized}<Shell>{#if app.data}{#key app.session?.id}{@render children()}{/key}{:else if app.loading}<div class="loading-screen"><span class="spinner"></span>Memuat data kampus…</div>{:else}<section class="panel"><Empty title="Data belum dapat dimuat" description="Periksa koneksi PocketBase lalu coba kembali."/><div class="center-actions"><button class="button" onclick={() => app.reload()}>Coba muat ulang</button></div></section>{/if}</Shell>{:else}<main id="main-content" class="loading-screen"><span class="spinner"></span>Menyiapkan ruang kerja…</main>{/if}
