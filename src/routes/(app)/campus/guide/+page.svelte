...
<div class="app-guide">
  <section class="guide-hero" aria-labelledby="guide-title">
    <img src="/logo-pf-white.png" alt="Pertamina Foundation" width="205" height="55" />
    <span class="guide-eyebrow">KENALI RUANG KERJA ANDA</span>
    <h1 id="guide-title">Langkah pertama menuju<br />dampak bersama.</h1>
    <p>
      Digitalisasi DEB membantu kampus mitra memantau capaian, mengelola proposal, dan belajar
      bersama dalam satu tempat.
    </p>
    <a class="guide-start" href="/campus/dashboard"
      >Mulai dari Beranda <Icon name="arrow" size={18} /></a
    >
  </section>

  <section aria-labelledby="features-title">
    <div class="guide-heading">
      <h2 id="features-title">Apa yang bisa Anda lakukan?</h2>
      <p>
    ...
    <div class="feature-grid">
      {#each features as feature}
        <article class="feature-card">
          <span class="feature-symbol"><Icon name={feature.icon} size={24} /></span>
          <h3>{feature.title}</h3>
          <p>{feature.description}</p>
          <a href={`/campus/${feature.href}`}>{feature.action}<Icon name="arrow" size={16} /></a>
        </article>
      {/each}
    </div>
  </section>
  ...
  <aside class="guide-tip">
    <span class="tip-icon"><Icon name="leaf" size={25} /></span>
    <div>
      <h2>Mulai sedikit, lanjutkan bersama.</h2>
      <p>
...
<style>
  ...
  .guide-hero {
    ...
    padding: 36px;
    border-radius: 18px;
    background: linear-gradient(120deg, #0878d8, #2040b5);
    color: white;
  }
  .guide-hero::after {
    content: '';
