...
<div class="page-heading">
  <div>
    <span class="eyebrow">{isAdmin ? 'MONITORING KAMPUS MITRA' : 'RUANG KERJA KAMPUS'}</span>
    <h1>{isAdmin ? 'Setiap langkah, berarti.' : `Selamat datang, ${app.session?.name}.`}</h1>
    <p>
      {isAdmin
  ...
  <span class="period"><Icon name="clock" size={16} />Pembacaan lokal</span>
</div>
<section class="hero-banner">
  <div class="hero-copy">
    <span class="hero-label"><span></span>PROGRAM DEB PUTIH</span>
    <h2>
    ...
    <a class="button mint" href={isAdmin ? '/admin/campuses' : '/campus/indicators'}
  ...
  <div class="hero-visual" aria-hidden="true">
    ...
    <div class="hero-seed"><Icon name="leaf" size={70} /></div>
    <span class="floating-card top"
      ><Icon name="campus" size={18} />{app.data!.campuses.length} kampus, satu visi</span
    ><span class="floating-card bottom"
      ><span class="mini-check"><Icon name="check" size={14} /></span>Tumbuh berkelanjutan</span
    >
  </div>
</section>
...
<div class="dashboard-columns">
  <section class="panel">
    <div class="panel-heading">
      <div>
        <h2>{isAdmin ? 'Perjalanan kampus mitra' : 'Capaian per bidang'}</h2>
        <p>
          {isAdmin
      ...
      <a class="text-link" href={isAdmin ? '/admin/campuses' : '/campus/indicators'}
        >Lihat semua<Icon name="arrow" size={16} /></a
      >
    </div>
    {#if isAdmin}<CampusTable compact />{:else}<div class="category-progress">
        {#each [...new Set(app.data!.definitions.map((d) => d.category))] as category, index}{@const definitions =
          ...
          <div class="category-row">
            <span class={`category-icon category-${index % 3}`}
              ><Icon name={['indicators', 'leaf', 'campus'][index % 3]} /></span
            >
            <div>
              <div class="row-between">
                <strong>{category}</strong><strong>{percent(p)}</strong>
              </div>
  ...
  <section class="panel next-step">
    ...
    <div class="next-icon"><Icon name="leaf" size={30} /></div>
    <h2>{isAdmin ? 'Arahan kecil,\nkemajuan berarti.' : 'Lengkapi cerita\nperkembangan Anda.'}</h2>
    <p>
      {isAdmin
    ...
    <a class="button secondary" href={isAdmin ? '/admin/verifikasi' : '/campus/indicators'}
      >{isAdmin ? 'Review pengajuan kampus' : 'Lihat feedback'}<Icon name="arrow" size={16} /></a
    >
  </section>
</div>
<div class="dashboard-columns equal">
  <section class="panel">
    <div class="panel-heading">
      <div>
        <h2>Aktivitas terbaru</h2>
        <p>Jejak langkah dalam ruang kerja.</p>
      </div>
      <Icon name="clock" size={19} />
    </div>
    <div class="activity-list">
      {#each activity as a}<div class="activity">
  ...
  <section class="panel">
    <div class="panel-heading">
      <div>
        <h2>Belajar dari pertanyaan bersama</h2>
        <p>Percakapan yang membantu kampus bertumbuh.</p>
      </div>
      <a class="text-link" href={`${prefix}/questions`}>Buka forum<Icon name="arrow" size={16} /></a
      >
    </div>
    <div class="popular-list">
      {#each popular as q}<a href={`${prefix}/questions/${q.id}`}
          ><span class="popular-like"
            ><Icon name="like" size={15} />{app.data!.likes.filter((l) => l.questionId === q.id)
              .length}</span
          ><span
            ><strong>{q.title}</strong><small
