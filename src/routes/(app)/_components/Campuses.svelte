...
{#if !id}<div class="page-heading">
    <div>
      <span class="eyebrow">JEJARING KOLABORASI</span>
      <h1>Kampus mitra</h1>
      <p>
  ...
  <div class="master-toolbar"><CampusMaster /></div>
  <section class="panel"><CampusTable /></section>{:else}<a class="back-link" href="/admin/campuses"
    >← Kembali ke kampus mitra</a
  >{#if !campus}<section class="panel">
      <Empty
        title="Kampus tidak ditemukan"
        description="Periksa kembali alamat kampus yang Anda buka."
        icon="campus"
      />
    </section>{:else}<div class="page-heading">
      <div class="campus-detail-title">
        <span class="large-avatar" style:font-size={campus.initials.length > 4 ? '13px' : undefined}
          >{campus.initials}</span
        >
        <div>
          <span class="eyebrow">KAMPUS MITRA · {campus.region.toUpperCase()}</span>
          <h1>{campus.name}</h1>
          <p>Perkembangan dan dokumentasi program DEB Putih.</p>
        </div>
      </div>
    ...
    <div class="master-toolbar"><CampusMaster {campus} /></div>
    <div class="tabs page-tabs">
      {#each ['Ringkasan', 'Indikator', 'Proposal', 'Feedback'] as t}<button
          class:active={tab === t}
          onclick={() => selectTab(t)}
          >{t}{#if t === 'Feedback'}<span class="count">{feedback.length}</span>{/if}</button
        >{/each}
    </div>
    {#if tab === 'Ringkasan'}<div class="stats-grid">
        <Stat
          label="Progres DEB"
      ...
      <section class="panel detail-overview">
        <h2>Pendampingan yang terarah</h2>
        <p>
          Lihat capaian indikator, telusuri proposal, dan berikan feedback sesuai perkembangan
          kampus.
        </p>
        <div class="button-row">
          <button class="button" onclick={() => selectTab('Indikator')}
            >Tinjau indikator<Icon name="arrow" size={17} /></button
          ><button class="button secondary" onclick={() => selectTab('Proposal')}
      ...
      >
        <div class="panel-heading">
          <div>
            <h2>Riwayat feedback kampus</h2>
            <p>Feedback selalu terhubung dengan indikator yang ditinjau.</p>
          </div>
        </div>
        {#if feedback.length}<div class="feedback-list padded">
            {#each feedback as f}{@const indicator = app.data!.indicators.find(
              ...
              <article class="feedback-card">
                ...
                <p>{f.text}</p>
                <div class="row-between">
                  <small>{date(f.createdAt)} · Admin PF</small><button
                    class="text-link"
