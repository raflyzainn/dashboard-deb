...
{#snippet categoryFilters()}
  <div class="topic-filter">
    <div class="topic-filter-title">
      <strong>Filter kategori topik</strong>{#if categoryFilter.length}<button
          class="text-link"
    ...
    <p>Pilih satu atau beberapa kategori. Hasil mencakup salah satu kategori yang dipilih.</p>
  </div>
{/snippet}
{#if id}
  <a class="back-link" href={prefix}>← Kembali ke forum bersama</a>
  {#if !question}<section class="panel">
      <Empty
    ...
    <div class="forum-detail">
      <div class="thread-card">
        <article class="panel thread-row question-thread">
          ...
          <div class="thread-content">
            <div class="thread-meta">
              <strong>{author(question.campusId)?.name}</strong><small
                >{date(question.createdAt)}</small
              >
            </div>
            <span class="thread-origin">Pertanyaan awal</span>
            ...
            <div class="thread-actions">
              ...
              {#if !isAdmin}<button
                ...
                >{:else}<span class="muted">{likeCount(id)} suka</span>{/if}
            </div>
          </div>
        </article>
        <section class="panel thread-row answer-thread" aria-label="Jawaban Admin PF">
          ...
          <div class="thread-content">
            <div class="thread-meta">
              <strong>Admin Pertamina Foundation</strong>{#if answer}<Badge tone="green"
                  >Jawaban resmi</Badge
                >{/if}
            </div>
            {#if answer && !editingAnswer}
              ...
              {#if isAdmin}<div class="thread-actions">
                  <button
                    disabled={app.readOnly || app.loading || app.busy}
                    class="text-link"
                    onclick={() => {
...
{:else}
  <div class="page-heading">
    <div>
      <span class="eyebrow">BELAJAR BERSAMA</span>
      <h1>Forum Q&A</h1>
      <p>Satu pertanyaan, banyak pembelajaran. Terhubung dengan seluruh kampus mitra.</p>
    </div>
    {#if !isAdmin}<button
        disabled={app.readOnly || app.loading || app.busy}
        class="button"
  ...
  <div class="forum-banner">
    <span class="forum-banner-icon"><Icon name="questions" size={29} /></span>
    <div>
      <h2>Ruang bertanya. Ruang bertumbuh.</h2>
      <p>
  ...
  <div class="forum-layout">
    <section class="panel">
      <div class="toolbar">
        ...
        <select aria-label="Urutkan pertanyaan" bind:value={order}
      ...
      <div class="tabs">
        <button class:active={status === 'all'} onclick={() => (status = 'all')}
          >Semua pertanyaan <span class="count">{app.data!.questions.length}</span></button
        ><button class:active={status === 'unanswered'} onclick={() => (status = 'unanswered')}
          >Belum dijawab</button
        ><button class:active={status === 'waiting'} onclick={() => (status = 'waiting')}
          >Menunggu tanggapan admin</button
        ><button class:active={status === 'answered'} onclick={() => (status = 'answered')}
          >Sudah dijawab</button
        >
      </div>
      {#if questions.length}<div class="question-list">
          {#each questions as q}<article>
              <div class="question-vote">
                {#if !isAdmin}<button
                    class="like-button vertical"
                    ...
                    onclick={() =>
                      ...
                      )}><Icon name="like" size={19} /><strong>{likeCount(q.id)}</strong></button
                  >{:else}<span class="like-button vertical"
                    ><Icon name="like" size={19} /><strong>{likeCount(q.id)}</strong></span
                  >{/if}
              </div>
              <div class="question-summary">
                ...
                <a href={`${prefix}/${q.id}`}><h3>{q.title}</h3></a>
                <p>{q.body}</p>
                ...
                <div class="question-tags">
                  ...
                  ><span class="muted">{q.replyCount || 0} balasan</span><a
                    class="text-link"
                    href={`${prefix}/${q.id}`}
    ...
    <aside>
      <section class="panel forum-guide">
        <span class="eyebrow">BERBAGI DENGAN BAIK</span>
        <h3>Pertanyaan Anda bisa membantu yang lain.</h3>
        <p>
      ...
      <a class="faq-promo" href={`/${app.session?.role}/faq`}
        ><Icon name="faq" size={25} /><strong>Mungkin jawabannya<br />sudah ada di FAQ.</strong
...
{#if compose}<Modal
    ...
    ><p class="muted">Pertanyaan Anda dapat dibaca oleh seluruh kampus dan Admin PF.</p>
    <form
      onsubmit={(e) => {
        e.preventDefault();
        ask();
    ...
    >
      <fieldset class="topic-fieldset">
        <legend>Kategori pertanyaan</legend>
        <p>Pilih setidaknya satu kategori yang sesuai.</p>
...
<style>
  ...
  .topic-filter-title strong {
    font-size: 12px;
  }
  .topic-filter-title button {
    font-size: 10px;
  }
  .topic-buttons {
    display: flex;
