...
<div class="page-heading">
  <div>
    <span class="eyebrow">KABAR RUANG KERJA</span>
    <h1>Notifikasi</h1>
    <p>Ikuti feedback, pembaruan data, proposal, dan percakapan yang memerlukan perhatian Anda.</p>
  </div>
  <button
...
<section class="panel notification-panel">
  <div class="notification-top">
    <div>
      <strong>{unread} belum dibaca</strong>
      <p>
  ...
  <div class="tabs">
    <button class:active={!unreadOnly} onclick={() => (unreadOnly = false)}
      >Semua <span class="count">{notices.length}</span></button
    ><button class:active={unreadOnly} onclick={() => (unreadOnly = true)}
      >Belum dibaca <span class="count">{unread}</span></button
    >
  </div>
  {#if visible.length}<div class="notification-list">
      {#each visible as notice}<article class:unread={!notice.readAt}>
          <span class="notification-icon"><Icon name="notifications" size={21} /></span>
          <div class="notification-copy">
            <div class="notification-heading">
              <h2>{notice.title}</h2>
              {#if notice.simulated}<span class="simulation-label">Simulasi</span
                >{/if}{#if !notice.readAt}<span class="unread-label">Belum dibaca</span>{/if}
            </div>
            <p>{notice.body}</p>
            <small
