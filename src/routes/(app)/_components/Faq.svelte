...
<div class="page-heading">
  <div>
    ...
    <h1>Jawaban untuk langkah Anda.</h1>
    <p>Temukan panduan singkat seputar indikator, proposal, dan ruang kerja DEB.</p>
  </div>
  {#if isAdmin}<button
...
<section class="faq-header">
  <div class="faq-header-icon"><Icon name="faq" size={36} /></div>
  <h2>Apa yang ingin Anda ketahui?</h2>
  <p>Pengetahuan bersama yang dikurasi oleh Admin PF.</p>
...
<div class="faq-content">
  <div class="row-between faq-count">
    <h2>Pertanyaan yang sering ditanyakan</h2>
    <span class="count">{entries.length} panduan</span>
  </div>
  {#if entries.length}<div class="faq-list">
      {#each entries as f (f.id)}<div class="faq-entry">
          <details>
            <summary><span>{f.question}</span><Icon name="plus" size={20} /></summary>
            <div class="faq-answer pre-wrap">{f.answer}</div>
          </details>
          {#if isAdmin}<div class="faq-admin">
  ...
  <div class="faq-contact">
    ...
    <div>
      ...
      <p>Bagikan di forum agar seluruh kampus bisa belajar bersama.</p>
    </div>
    <a class="button secondary" href={`/${app.session?.role}/questions`}
      >Buka forum<Icon name="arrow" size={17} /></a
    >
  </div>
</div>
{#if editing}<Modal
    ...
    >
      ...
      >
      <p class="muted">Isi FAQ dikelola terpisah dari jawaban di forum.</p>
      <div class="dialog-actions">
        <button type="button" class="button secondary" onclick={() => (editing = false)}
          >Batal</button
...
{#if removing}<Modal
    ...
    ><p>{removing.question}</p>
    <p class="muted">
      FAQ akan dihapus dari pusat bantuan. Pertanyaan dan jawaban sumber di forum tetap tersedia.
    </p>
    <div class="dialog-actions">
      <button class="button secondary" onclick={() => (removing = null)} disabled={app.busy}
        >Batal</button
      ><button
        class="button danger"
        disabled={app.readOnly || app.loading || app.busy}
