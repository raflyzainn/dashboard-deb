...
{#if !embedded}<div class="page-heading">
    <div>
      <span class="eyebrow">CAPAIAN PROGRAM</span>
      <h1>Indikator DEB Putih</h1>
      <p>
        {isAdmin
...
{#if isAdmin}<div class="info-note">
    <Icon name="verifikasi" /><span
      ><strong>Data isian kampus ? Baca saja</strong><br />Admin meninjau dan memberi feedback.
      Perbaikan nilai hanya dilakukan oleh kampus.</span
    >
  </div>{/if}
<section class="panel">
  <div class="toolbar">
    ...
    {#if isAdmin && !campusId}<select aria-label="Pilih kampus" bind:value={campus}
        >{#each app.data!.campuses as c}<option value={c.id}>{c.name}</option>{/each}</select
      >{/if}<select aria-label="Filter status indikator" bind:value={status}
  ...
  {#if rows.length}<div class="table-scroll">
      <table>
        <thead
          ><tr
            ><th>INDIKATOR</th><th>BASELINE</th><th>TARGET</th><th>AKTUAL</th><th>PROGRES</th><th
              >STATUS</th
            ><th>AKSI</th></tr
          ></thead
        ><tbody
          >{#each rows as i}{@const d = definition(i.definitionId)}{@const revise =
              ...
              ><td
                ><strong class="table-title">{d.name}</strong><small class="table-subtitle"
...
{#if selected}<Modal
    ...
    wide
    ><p class="muted">{definition(selected.definitionId).description}</p>
    <div class="mini-stats">
      <div><small>Baseline</small><strong>{selected.baseline}</strong></div>
      <div><small>Target</small><strong>{selected.target}</strong></div>
      <div><small>Satuan</small><strong>{definition(selected.definitionId).unit}</strong></div>
    </div>
    {#if !isAdmin}<form
        onsubmit={(e) => {
          e.preventDefault();
    ...
    <h3>Feedback Admin PF <span class="count">{comments.length}</span></h3>
    {#if comments.length}<div class="feedback-list">
        {#each comments as f}<article class="feedback-card">
            <div class="row-between">
              <strong>Admin PF</strong><Badge
                tone={f.state === 'closed' ? 'green' : f.state === 'responded' ? 'blue' : 'amber'}
                >{f.requiresRevision ? feedbackLabel[f.state] : 'Catatan'}</Badge
              >
            </div>
            <p class="pre-wrap">{f.text}</p>
            <div class="row-between">
