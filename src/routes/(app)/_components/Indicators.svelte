<script lang="ts">
  ...
  async function save() {
    ...
    if (await app.mutate(() => dataService.updateIndicator(app.session!, selected!.id, current!, note), 'Indikator berhasil diperbarui.')) selected = null;
  }
  async function comment() { if (await app.mutate(() => dataService.addFeedback(app.session!, selected!.id, feedback, revision), 'Feedback berhasil dikirim.')) feedback = ''; }
</script>
{#if !embedded}<div class="page-heading"><div><span class="eyebrow">CAPAIAN PROGRAM</span><h1>Indikator DEB Putih</h1><p>{isAdmin ? 'Tinjau perkembangan kampus dan berikan arahan yang sesuai.' : 'Catat perkembangan nyata, satu indikator setiap langkah.'}</p></div><Badge tone="green">30 indikator simulasi</Badge></div>{/if}
