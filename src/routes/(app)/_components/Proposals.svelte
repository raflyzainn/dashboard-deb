<script lang="ts">
  ...
  let destroyed = false;
  onDestroy(() => { destroyed = true; closePreview(); });
  async function view(proposal: ProposalVersion) {
    fileLoading = true;
    try { const blob = await dataService.proposalFile(proposal.id); if (!destroyed) { closePreview(); preview = { url: URL.createObjectURL(blob), proposal }; } }
    catch (e) { if (!destroyed) app.error = e instanceof Error ? e.message : 'PDF tidak dapat dibuka.'; }
    finally { fileLoading = false; }
  }
  async function save() {
    if (!file) { app.error = 'Pilih PDF untuk diunggah.'; return; }
    if (await app.mutate(() => dataService.uploadProposal(file!, changes), 'Versi baru proposal berhasil diajukan.')) { upload = false; file = null; changes = ''; }
  }
</script>
{#snippet comparison()}
  {#key activeCampus}<ProposalCompare {versions} actor={app.session!} onview={view}/>{/key}
{/snippet}
{#if !embedded}<div class="page-heading"><div><span class="eyebrow">DOKUMEN PROGRAM</span><h1>Proposal kampus</h1><p>{isAdmin ? 'Ikuti perkembangan gagasan dan rencana kerja kampus mitra.' : 'Simpan rencana kerja Anda, lengkap dengan setiap jejak perubahannya.'}</p></div>{#if !isAdmin}<button disabled={app.readOnly || app.loading || app.busy} class="button" onclick={() => { file = null; changes = ''; upload = true; }}><Icon name="plus" size={18}/>Unggah versi baru</button>{/if}</div>{/if}
