...
{/snippet}
{#if !embedded}<div class="page-heading">
    <div>
      <span class="eyebrow">DOKUMEN PROGRAM</span>
      <h1>Proposal kampus</h1>
      <p>
        {isAdmin
...
{#if isAdmin && !campusId}<section class="panel proposal-selector">
    <div class="toolbar">
      ...
      <select aria-label="Filter proposal" bind:value={status}
    ...
    <div class="campus-picker">
      <label
        >Kampus yang ditinjau<select aria-label="Pilih kampus proposal" bind:value={campus}
          >{#each campuses as c}<option value={c.id}
            ...
            >{/each}</select
        ></label
      ><span class="muted">{campuses.length} kampus sesuai filter</span>
    </div>
    {#if !campuses.length}<Empty
        title="Kampus tidak ditemukan"
        description="Ubah kata pencarian atau status proposal."
      />{:else if !campuses.some((c) => c.id === campus)}<p class="inline-hint">
        Pilih kampus dari hasil filter untuk meninjau proposal.
      </p>{/if}
  </section>{/if}
{#if !isAdmin || campusId || campuses.some((c) => c.id === campus)}<div class="proposal-grid">
    <div>
      <section class="panel">
        <div class="panel-heading">
          <div>
            <h2>Proposal terbaru</h2>
            <p>{app.data!.campuses.find((c) => c.id === activeCampus)?.name}</p>
          </div>
          <Badge tone={current ? 'green' : 'neutral'}
            >{current ? 'Diajukan' : 'Belum diunggah'}</Badge
          >
        </div>
        {#if current}<div class="proposal-current">
            <div class="pdf-cover">
              <span>DEB PUTIH</span><Icon name="proposal" size={58} /><strong
                >PROPOSAL<br />PROGRAM</strong
              ><small>Versi {current.version}</small>
              <div class="pdf-cover-line"></div>
            </div>
            <div class="proposal-info">
              <span class="eyebrow">DOKUMEN AKTIF · V{current.version}</span>
              <h3>{current.filename}</h3>
              <p>{date(current.createdAt)} · {size(current.size)} · PDF</p>
              <div class="button-row">
                <button class="button" disabled={fileLoading} onclick={() => view(current)}
                  ><Icon name="eye" size={17} />Lihat proposal</button
                >{#if !isAdmin}<button
                    disabled={app.readOnly || app.loading || app.busy}
      ...
      <section class="panel history-panel">
        <div class="panel-heading">
          <div>
            <h2>Riwayat versi</h2>
            <p>Setiap perubahan adalah bagian dari perjalanan.</p>
          </div>
          <span class="count">{versions.length} versi</span>
        </div>
        {#if versions.length}<div class="version-list">
            {#each versions as v, index}<article class="version-item">
                <div class="version-node"><Icon name="proposal" size={19} /></div>
                <div class="version-content">
                  <div class="row-between">
                    <div>
                      <strong>Versi {v.version}</strong>{#if index === 0}<Badge tone="green"
                          >Terbaru</Badge
                        >{/if}
                    </div>
                    <button
                  ...
                  <small>{date(v.createdAt)} · {size(v.size)}</small>
    ...
    <aside class="panel guidance">
      <div class="guidance-icon"><Icon name="leaf" size={25} /></div>
      <h3>Rencana yang terus bertumbuh.</h3>
      <p>
...
{#if upload}<Modal
    ...
    >
      <label class="upload-zone"
        ><Icon name="upload" size={32} /><strong>Pilih dokumen proposal</strong><span
          >PDF · Maksimal 10 MiB</span
        ><input
          type="file"
...
{#if preview}<Modal title={`Proposal versi ${preview.proposal.version}`} onclose={closePreview} wide
    ><div class="row-between preview-heading">
      <p>{preview.proposal.filename}</p>
      <a class="button secondary small" href={preview.url} download={preview.proposal.filename}
        ><Icon name="download" size={16} />Unduh PDF</a
      >
    </div>
    <iframe class="pdf-preview" src={preview.url} title={`Pratinjau ${preview.proposal.filename}`}
    ></iframe>
    <p class="muted">Jika pratinjau tidak didukung browser, gunakan Unduh PDF.</p></Modal
  >{/if}
