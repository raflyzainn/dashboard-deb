<script lang="ts">
  ...
  async function view(proposal: ProposalVersion) {
    fileLoading = true;
    try { const blob = await dataService.proposalFile(app.session!, proposal.id); if (!destroyed) { closePreview(); preview = { url: URL.createObjectURL(blob), proposal }; } }
