<script lang="ts">
  ...
  function wheelMap(event: WheelEvent) {
    ...
    if ((direction > 0 && zoom >= 3) || (direction < 0 && zoom <= 1)) return;
    event.preventDefault();
    setZoom(zoom + direction * 0.2);
  }
...
<div class="map-layout">
  <section class="panel map-panel">
    ...
    <div bind:this={mapStage} class="map-stage" class:dragging role="img" aria-label={`Peta Indonesia dengan ${visible.length} titik kampus mitra`} onwheel={wheelMap} onpointerdown={startPan} onpointermove={movePan} onpointerup={endPan} onpointercancel={endPan}>
