<script lang="ts">
  ...
  import { mapCampuses, layoutMapPoints, progressBands, regionSummary, type ProgressBand } from '$lib/map';
  ...
  const markers = $derived(layoutMapPoints(visible, stageWidth * .94, stageWidth * .94 * .52, zoom, stageHeight));
  ...
  function panLimit(nextZoom = zoom) {
    ...
    return {
      x: Math.max(0, (stageWidth * .94 * nextZoom - stageWidth) / 2),
      y: Math.max(0, (stageWidth * .94 * .52 * nextZoom - stageHeight) / 2)
    };
  }
  ...
  $effect(() => { const limit = panLimit(); clampPan(Math.max(-limit.x, Math.min(limit.x, panX)), Math.max(-limit.y, Math.min(limit.y, panY))); });
...
<div class="map-layout">
  <section class="panel map-panel">
    <header><div><h2>Sebaran geografis</h2><p>Ketuk titik untuk memilih kampus. Gunakan +/− untuk zoom, lalu geser peta.</p></div><span>{visible.length} kampus</span></header>
    <div bind:this={mapStage} bind:clientWidth={stageWidth} bind:clientHeight={stageHeight} class="map-stage" class:dragging class:wheel-zooming={wheelZooming} role="group" aria-label={`Peta Indonesia dengan ${visible.length} titik kampus mitra`} onwheel={wheelMap} onpointerdown={startPan} onpointermove={movePan} onpointerup={endPan} onpointercancel={endPan}>
      ...
      <svg class="marker-links" viewBox="0 0 1000 520" aria-hidden="true">
        {#each markers as point (point.id)}
          {#if Math.abs(point.markerX - point.x) + Math.abs(point.markerY - point.y) > .01}
            <line x1={point.x * 10} y1={point.y * 5.2} x2={point.markerX * 10} y2={point.markerY * 5.2} vector-effect="non-scaling-stroke"/>
          {/if}
        {/each}
      </svg>
      {#each markers as point (point.id)}
        <button class="map-dot" class:selected={point.id === selectedId} style={`left:${point.markerX}%;top:${point.markerY}%;--dot:${progressBands[point.band].color};--size:13px`} aria-label={`${point.name}, ${point.province}, progres ${number(point.score)} persen`} title={point.name} onclick={() => selectedId = point.id}><span></span></button>
      {/each}
      </div>
    </div>
    {#if selected}<article class="map-card" aria-label="Detail kampus terpilih"><div class="map-card-top"><span class="map-avatar">{selected.initials}</span><button aria-label="Tutup detail titik" onclick={() => { selectedId = ''; }}>×</button></div><strong>{selected.name}</strong><small>{selected.province} · {selected.island}</small><div class="row-between"><span>{progressBands[selected.band].label}</span><b>{number(selected.score)}%</b></div><Progress value={selected.score} label={`Progres ${selected.name}`}/><a href={`/admin/campuses/${selected.id}`}>Lihat detail kampus <Icon name="arrow" size={14}/></a></article>
    {/if}
    ...
    <p class="map-note"><Icon name="faq" size={15}/>Setiap titik mewakili satu kampus. Garis tipis menghubungkan titik yang direnggangkan ke lokasi aslinya.</p>
  </section>
...
<style>
  ...
  @media(max-width:700px){.page-heading{align-items:flex-start}.map-stats{gap:8px}.map-stats article{padding:13px;gap:9px}.stat-icon{width:32px;height:32px}.map-stats strong{font-size:21px}.map-stats p{display:none}.map-filters{grid-template-columns:1fr;padding:16px}.map-filters>div{grid-column:auto}.map-stage{min-height:310px;aspect-ratio:auto}.map-card{margin:12px;width:auto}.map-legend{grid-template-columns:repeat(2,minmax(0,1fr));padding:12px}.map-note{margin:0 12px 14px;line-height:1.6}.region-list{grid-template-columns:1fr}.map-panel>header,.region-panel>header{padding:17px}.readonly{align-self:flex-start}}

  .marker-links{overflow:visible;position:absolute;inset:0;width:100%;height:100%;pointer-events:none}.marker-links line{stroke:#7796b8;stroke-width:1;stroke-opacity:.65}
</style>
