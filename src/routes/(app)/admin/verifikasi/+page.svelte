...
<div class="page-heading">
  <div>
    <span class="eyebrow">RUANG REVIEW KAMPUS</span>
    <h1>Tinjauan &amp; Konfirmasi DEB</h1>
    <p>
...
<div class="review-status-strip" aria-label="Ringkasan review">
  {#each [{ value: 'pending', label: 'Menunggu tinjauan', icon: 'clock', note: 'Siap diperiksa Admin PF' }, { value: 'approved', label: 'Sudah dikonfirmasi', icon: 'check', note: 'Tersimpan dalam riwayat' }, { value: 'revision', label: 'Dikembalikan ke kampus', icon: 'edit', note: 'Perbaikan oleh pemilik data' }] as item}<button
      ...
      ><span class={`status-icon ${item.value}`}><Icon name={item.icon} /></span><span
        class="status-copy"
        ><small>{item.label}</small><strong
          >{submissions.filter((s) => s.status === item.value).length}<span>pengajuan</span></strong
...
<div class="verification-layout">
  <section class="panel queue" aria-label="Daftar pengajuan">
    <div class="queue-heading">
      <h2>Kiriman kampus</h2>
      <p>Paling lama menunggu ditampilkan lebih dulu.</p>
      <label class="search-field"
        ><Icon name="search" size={17} /><input
          ...
          bind:value={search}
        /></label
      ><select aria-label="Status pengajuan" bind:value={status}
    ...
    <div class="queue-list">
      {#each rows as row}<button
          ...
          ><strong>{campus(row.campusId).name}</strong><span>{campus(row.campusId).region}</span>
          <div>
            <Badge
            ...
            ><small>{row.simulated ? 'Simulasi' : 'Isian kampus'}</small>
          </div>
          <small>Dikirim {date(row.submittedAt)}</small></button
        >{:else}<p class="queue-empty">Tidak ada pengajuan yang cocok.</p>{/each}
    </div>
  </section>
    ...
    {#if selected}
      <section class="panel review-summary">
        <div class="row-between">
          <div>
            <span class="eyebrow">PENGAJUAN #{selected.version}</span>
            <h2>{campus(selected.campusId).name}</h2>
            <p>{campus(selected.campusId).region} · Dikirim {date(selected.submittedAt)}</p>
          </div>
          <Badge
        ...
        <div class="review-metrics">
          <div><small>Indikator dikirim</small><strong>{selected.indicators.length}</strong></div>
          <div><small>Target belum tercapai</small><strong>{gaps.length}</strong></div>
          <div><small>Feedback revisi aktif</small><strong>{unresolved.length}</strong></div>
        </div>
        <div class="info-note">
          <Icon name="verifikasi" />
          <div>
            <strong>Admin meninjau dan mengonfirmasi</strong>
            <p>
        ...
        <a class="text-link" href={`/admin/campuses/${selected.campusId}`}
          >Buka profil dan proposal kampus <Icon name="arrow" size={15} /></a
        >
      </section>
      <section class="panel gap-panel">
        <div class="row-between">
          <h2>Kesenjangan terhadap target</h2>
          <small>Target simulasi</small>
        </div>
        {#if gaps.length}<p>
      ...
      {#if selected.status === 'pending'}
        <div class="review-indicators">
          <h2>Indikator yang ditinjau</h2>
          <p>Buka Tinjau untuk membaca catatan kampus dan memberi feedback per indikator.</p>
          {#key selected.id}<Indicators campusId={selected.campusId} embedded />{/key}
        </div>
        <section class="panel comments-panel">
          <div class="row-between">
            <h2>Feedback kampus ini</h2>
          ...
          {#each comments as comment}<article>
              ...
              {#if comment.state !== 'closed'}<button
                ...
                >{/if}
            </article>{:else}<p>Belum ada feedback untuk kampus ini.</p>{/each}
        </section>
        <section class="panel decision-panel">
          <h2>Hasil tinjauan</h2>
          <p>Tinjau seluruh data dan feedback sebelum memberikan keputusan.</p>
          <label
            ...
            ></textarea></label
          >{#if unresolved.length}<p class="decision-help">
      ...
      {:else}
        <section class="panel decision-panel">
          <h2>Keputusan Admin PF</h2>
          <p>{selected.reviewedBy} · {date(selected.reviewedAt!)}</p>
          <p class="pre-wrap">{selected.decisionNote || 'Data pengajuan telah disetujui.'}</p>
          <details>
            <summary>Lihat arsip nilai yang dikirim</summary>
            <div class="table-scroll">
              <table>
                <thead
                  ><tr><th>Indikator</th><th>Aktual</th><th>Target</th><th>Catatan kampus</th></tr
                  ></thead
                ><tbody
                  >{#each selected.indicators as item}<tr
                      ><td>{item.name}</td><td>{number(item.current)}</td><td
                        >{number(item.target)}</td
                      ><td class="pre-wrap">{item.note || '—'}</td></tr
                    >{/each}</tbody
                >
              </table>
            </div>
...
{#if decision && selected}<Modal
    ...
    ><p><strong>{campus(selected.campusId).name} · Pengajuan #{selected.version}</strong></p>
    <p>
      {decision === 'approved'
    ...
    {#if note.trim()}<p class="pre-wrap">{note}</p>{/if}
    <div class="dialog-actions">
      <button class="button secondary" disabled={app.busy} onclick={() => (decision = null)}
        >Batal</button
      ><button class="button" disabled={app.readOnly || app.loading || app.busy} onclick={review}
        >Konfirmasi keputusan</button
      >
    </div></Modal
  >{/if}
...
<style>
  ...
  .review-status-strip > button:hover,
  .review-status-strip > button.current {
    border-color: #8aa474;
    box-shadow: 0 4px 14px #29442108;
  }
  .status-icon {
    width: 40px;
    height: 40px;
  ...
  .review-summary .info-note {
    margin: 0 0 18px;
  }
  .review-summary .info-note p {
    margin-bottom: 0;
  }
  .gap-list {
    display: grid;
  ...
  .review-indicators :global(.toolbar) {
    flex-wrap: wrap;
    padding: 16px;
    gap: 10px;
  }
  .review-indicators :global(.category-tabs) {
    overflow-x: auto;
    white-space: nowrap;
    padding: 0 16px;
  }
  .review-indicators :global(.table-scroll table) {
    min-width: 760px;
  }
  ...
  .comments-panel article:first-of-type {
    margin-top: 16px;
  }
  .comments-panel article strong {
    font-size: 12px;
  }
  ...
  @media (max-width: 800px) {
    ...
    .status-icon,
    .review-status-strip > button > :global(svg),
    .status-copy > span {
      display: none;
    }
    .status-copy strong {
  ...
  @media (max-width: 600px) {
    ...
    .decision-panel .button-row .button {
      width: 100%;
    }
  }
</style>
