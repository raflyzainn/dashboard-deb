    ...
    <section class="login-story">
      ...
      <div class="login-copy">
        ...
        <div class="login-metrics">
          <div>
            <strong>{app.accounts.filter((a) => a.role === 'campus').length || '—'}</strong><span
              >Kampus mitra</span
            >
          </div>
      ...
      <div class="login-form">
        ...
        >
          ...
          >
            ...
            {:else}{#if !app.accountsLoading}<p class="no-results">
                  {search
          ...
          <button class="button" disabled={!account || app.accountsLoading || app.loading}
            >{app.loading ? 'Memuat PocketBase…' : 'Buka ruang kerja'}<Icon name="arrow" /></button
          >
        </form>
        {#if app.accountsLoading}<p role="status">
            Memuat akun dari PocketBase…
          </p>{:else if !app.accounts.length}<p>
...
<style>
  ...
  .account-list {
    max-height: clamp(110px, calc(100dvh - 560px), 220px);
  }
  .account-card {
    padding: 10px;
  }
