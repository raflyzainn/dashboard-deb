<script lang="ts">
  ...
  $effect(() => { if (qa) untrack(() => { void app.loadAccounts(); }); });
  ...
  $effect(() => { if (!page.url.searchParams.get('token') && app.ready && app.session) goto(`/${app.session.role}/dashboard`, { replaceState: true }); });
