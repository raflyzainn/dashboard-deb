...
{@render children()}
{#if app.error && !app.dialogs}<div class="global-error" role="alert">
    <Icon name="alert" />
    <div>
      <strong>Tindakan belum berhasil</strong>
      <p>{app.error}</p>
    </div>
    <button class="icon-button" aria-label="Tutup pesan kesalahan" onclick={() => (app.error = '')}
      ><Icon name="close" /></button
    >
  </div>{/if}
{#if app.toast && !app.dialogs}<div class="toast" role="status">
