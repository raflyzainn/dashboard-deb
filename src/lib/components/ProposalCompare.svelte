...
<section class="panel comparison" aria-label="Perbandingan proposal">
  <div class="panel-heading">
    <div>
      <span class="eyebrow">REVIEW PERUBAHAN</span>
      <h2>Bandingkan versi proposal</h2>
      <p>Pilih dua versi untuk melihat perubahan isi dokumen.</p>
    </div>
  ...
  {:else}
    <div class="compare-selectors">
      <label
        >Versi dasar<select aria-label="Versi dasar" bind:value={beforeId}
      ...
      >
      <label
        >Versi pembanding<select aria-label="Versi pembanding" bind:value={afterId}
          >{#each versions as version}<option value={version.id}
              >Versi {version.version} · {version.filename}</option
            >{/each}</select
        ></label
    ...
    {#if before && after}
      <div class="compare-files">
        <div>
          <strong>Dasar: versi {before.version}</strong><span
            >{before.filename} · {date(before.createdAt)}</span
          ><button class="text-link" onclick={() => onview(before)}>Buka PDF dasar</button>
        </div>
        <div>
          <strong>Pembanding: versi {after.version}</strong><span
            >{after.filename} · {date(after.createdAt)}</span
          ><button class="text-link" onclick={() => onview(after)}>Buka PDF pembanding</button>
        </div>
      </div>
    {/if}
    ...
    {:else if busy}
      <p class="compare-empty" role="status">
        <span class="spinner"></span> Membaca dan membandingkan isi PDF…
      </p>
    {:else if error}
      <div class="compare-error" role="alert">
        <p>{error}</p>
        <button class="button secondary small" onclick={() => retry++}>Coba lagi</button>
      </div>
    {:else if result}
      {#if result.before.emptyPages.length || result.after.emptyPages.length}
        <p class="compare-warning" role="status">
      ...
      <div class="compare-toolbar">
        <div class="diff-counts" aria-live="polite">
          <span class="count-added">+{result.diff.added} baris ditambahkan</span><span
            class="count-removed">−{result.diff.removed} baris dihapus</span
          >
        </div>
        <div class="compare-options">
          <div class="view-switch" aria-label="Tampilan perbedaan">
            <button
          ...
          <label class="checkbox-label"
            ><input type="checkbox" bind:checked={onlyChanges} />Hanya perubahan</label
          >
        </div>
      </div>
      {#if !result.diff.added && !result.diff.removed}<p class="compare-identical" role="status">
      ...
      <div class="diff-scroll" tabindex="0" role="region" aria-label="Hasil perbedaan teks PDF">
        {#if mode === 'split'}
          <table class="diff-table split">
            <colgroup
              ><col class="number-column" /><col /><col class="number-column" /><col /></colgroup
            ><thead
              ><tr
                ><th colspan="2">Dasar · versi {before?.version}</th><th colspan="2"
                  >Pembanding · versi {after?.version}</th
                ></tr
              ></thead
            ><tbody
              >{#each paired as row}<tr
                  ><td class="line-number" class:deleted={row.changed && !!row.before}
                    >{row.before?.number || ''}</td
                  ><td class:deleted={row.changed && !!row.before} class:absent={!row.before}
                    ><span class="diff-sign">{row.changed && row.before ? '−' : ' '}</span
                    >{#if row.before}<span class="line-text">{row.before.text}</span><small
                        class="page-ref">Hlm. {row.before.page}</small
                      >{/if}</td
                  ><td class="line-number" class:inserted={row.changed && !!row.after}
                    >{row.after?.number || ''}</td
                  ><td class:inserted={row.changed && !!row.after} class:absent={!row.after}
                    ><span class="diff-sign">{row.changed && row.after ? '+' : ' '}</span
                    >{#if row.after}<span class="line-text">{row.after.text}</span><small
                        class="page-ref">Hlm. {row.after.page}</small
                      >{/if}</td
                  ></tr
                >{/each}</tbody
            >
        ...
        {:else}
          <table class="diff-table unified">
            <thead><tr><th>Dasar</th><th>Baru</th><th>Isi teks</th></tr></thead><tbody
              >{#each rows as row}<tr
                  class:deleted={row.kind === 'removed'}
                  class:inserted={row.kind === 'added'}
                  ><td class="line-number">{row.before?.number || ''}</td><td class="line-number"
                    >{row.after?.number || ''}</td
                  ><td
                    ><span class="diff-sign"
                      >{row.kind === 'added' ? '+' : row.kind === 'removed' ? '−' : ' '}</span
                    ><span class="line-text">{row.after?.text ?? row.before?.text}</span><small
                      class="page-ref">Hlm. {row.after?.page ?? row.before?.page}</small
                    ></td
                  ></tr
                >{/each}</tbody
            >
...
<style>
  ...
  .compare-files > div + div {
    border-left: 1px solid #dce5df;
  }
  .compare-files strong {
    font-size: 12px;
  }
  .compare-files span {
    font-size: 11px;
  ...
  .split td:nth-child(2),
  .split td:nth-child(4) {
    text-align: left;
  }
  .split td:nth-child(3) {
    border-left: 1px solid #dce5df;
  }
  .unified th:nth-child(1),
  .unified th:nth-child(2) {
    width: 44px;
  }
  .diff-table .deleted,
  ...
  .diff-table .inserted,
  .diff-table tr.inserted td {
    background: #dafbe1;
    color: #116329;
  }
  .diff-table .absent {
    background: repeating-linear-gradient(135deg, #f7f9f8, #f7f9f8 5px, #eef2ef 5px, #eef2ef 6px);
  }
  .diff-sign {
  ...
  @media (max-width: 700px) {
    ...
    .comparison :global(.panel-heading) {
      padding-inline: 16px;
    }
  }
</style>
