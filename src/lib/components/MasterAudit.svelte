...
<section class="panel master-panel" aria-label="Riwayat perubahan master">
  <div class="panel-heading">
    <div>
      <h2>Riwayat perubahan master</h2>
      <p>Telusuri seluruh riwayat, termasuk perubahan yang sudah lama.</p>
    </div>
    <label class="search-label audit-search"
  ...
  <div aria-busy={loading}>
    ...
    {:else if error}<div class="master-error" role="alert">
        {error} <button class="button secondary small" onclick={() => retry++}>Coba lagi</button>
      </div>
    ...
    {:else}
      ...
      <div class="audit-list">
        {#each result.items as item (item.id)}<details>
            <summary
              ><strong>{String(item.after?.name || item.before?.name || item.entityId)}</strong
              ><span
            ...
            <div class="master-table-wrap">
              <table class="master-table">
                <thead><tr><th>Field</th><th>Sebelum</th><th>Sesudah</th></tr></thead><tbody
                  >{#each Object.keys(auditFields).filter((key) => item.before?.[key] !== item.after?.[key]) as key}<tr
                      ><td>{auditFields[key]}</td><td>{display(item.before?.[key])}</td><td
