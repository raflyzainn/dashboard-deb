...
<div class="page-heading">
  <div>
    <span class="eyebrow">ADMINISTRASI AKSES</span>
    <h1>Akun kampus</h1>
    <p>Simpan nama dan email PIC. Kampus meminta tautan aktivasi sendiri melalui halaman login.</p>
  </div>
</div>
<p class="mock-note">
  Setelah data disimpan, PIC memilih Aktivasi akun di halaman login dan memasukkan email yang
  terdaftar. Menyimpan data tidak mengirim email. <button class="text-link" onclick={() => load()}
    >Muat ulang status</button
  >
</p>
<div class="access-stats">
  <article><span>Kampus mitra</span><strong>{stats.total}</strong></article>
  <article><span>Email tersimpan</span><strong>{stats.email}</strong></article>
  <article><span>Menunggu aktivasi</span><strong>{stats.waiting}</strong></article>
  <article><span>Akun aktif</span><strong>{stats.active}</strong></article>
</div>
<section class="panel account-panel" aria-label="Daftar email kampus">
  <div class="roster-heading">
    <div>
      <h2>Daftar akun kampus</h2>
      <p>Satu alamat email untuk setiap kampus mitra.</p>
    </div>
    <button class="button secondary" disabled={busy || !changed.length || hasErrors} onclick={save}
      >Simpan perubahan{changed.length ? ` (${changed.length})` : ''}</button
    >
  </div>
  <div class="filters">
    ...
    <select aria-label="Filter status akun" bind:value={filter}
  ...
  <p class="filter-summary" role="status" aria-live="polite" aria-atomic="true">
    {#if error}Jumlah hasil belum dapat diperbarui.{:else if resultsPending}Memuat hasil filter…{:else}Menampilkan
      <strong>{total}</strong> dari <strong>{stats.total}</strong> kampus{#if filter !== 'all'}
        · {filter}{/if}{/if}
  </p>
  {#if changed.length}<p class="hint">
      Simpan perubahan nama dan email agar PIC dapat meminta tautan aktivasi menggunakan data
      terbaru.
    </p>{/if}
  {#if hasErrors}<p class="error" role="alert">
  ...
  <div class="roster-head">
    <span>Kampus mitra</span><span>Email kampus</span><span>Status & tindakan</span>
  </div>
  <div class="roster">
    {#each paged as account (account.campusId)}
      ...
      <article class="account-row" class:editing={isEditing(account)} aria-label={account.campus}>
        <div class="campus-cell">
          <span class="avatar"><Icon name="campus" /></span>
          <div class="campus-details">
            <strong>{account.campus}</strong>
            <div class="pic-field">
              {#if isEditing(account)}<label for={`pic-${account.campusId}`}>Nama PIC</label><input
                  id={`pic-${account.campusId}`}
                  aria-label={`Nama PIC ${account.campus}`}
                  placeholder="Nama penanggung jawab"
                  bind:value={names[account.campusId]}
                  oninput={() => (notice = '')}
                />{:else}<p class="saved-pic">{account.name || 'Nama PIC belum diisi'}</p>{/if}
            </div>
          </div>
        </div>
        <div class="email-cell">
          {#if isEditing(account)}<label for={`email-${account.campusId}`}>Email kampus</label
            ><input
            ...
            />{#if issue}<small id={`email-error-${account.campusId}`} class="field-error"
                >{issue}</small
              ...
              >{:else}<p class="saved-pic">Email belum diisi</p>{/if}{/if}
        </div>
  ...
  <nav class="pagination" aria-label="Halaman daftar akun">
    <span
...
{#if resetAccounts}<Modal title="Ubah email akun aktif?" onclose={() => (resetAccounts = null)}
    ...
    <ul class="recipient-list">
      {#each resetAccounts as a}<li>
          <strong>{a.campus}</strong><span
            >{a.email} → {normalizeEmail(drafts[a.campusId]) || 'Email dikosongkan'}</span
          >
        </li>{/each}
    </ul>
    <div class="dialog-actions">
      <button class="button secondary" onclick={() => (resetAccounts = null)}>Batal</button><button
        class="button"
        disabled={busy}
...
<style>
  ...
  .row-actions .badge.amber {
    color: #956000;
    background: #fff4d6;
  }
  .row-actions .badge.green {
    color: #187347;
    background: #e7f6ec;
  }
  .campus-details {
  ...
  .account-row {
    display: grid;
    grid-template-columns: minmax(220px, 1.1fr) minmax(200px, 1fr) minmax(185px, 0.8fr);
    gap: 22px;
    padding: 19px 24px;
    align-items: center;
  }
  ...
  @media (max-width: 1100px) {
    ...
    .roster-head,
    .account-row {
      grid-template-columns: minmax(180px, 1fr) minmax(170px, 1fr) minmax(160px, 0.8fr);
      gap: 14px;
    }
  }
  @media (max-width: 750px) {
  ...
  @media (min-width: 751px) {
    .account-row:not(.editing) .saved-pic {
      margin: 0;
      line-height: 1.7;
    }
    .account-row:not(.editing) .saved-email {
      display: block;
      line-height: 1.7;
    }
    .account-row {
    ...
    .account-row:not(.editing) {
      row-gap: 4px;
    }
    .account-row:not(.editing) .campus-details > strong {
      align-self: end;
    }
    .account-row:not(.editing) .campus-cell > .avatar {
      align-self: center;
      margin-top: 0;
    }
    .account-row .campus-cell {
      grid-column: 1;
      grid-row: 1 / span 2;
      display: grid;
    ...
    .account-row .campus-cell > .avatar {
      grid-column: 1;
      grid-row: 1 / span 2;
    }
    .account-row .campus-details {
      grid-column: 2;
      grid-row: 1 / span 2;
      display: grid;
      grid-template-rows: subgrid;
    }
    .account-row .email-cell {
      grid-column: 2;
      grid-row: 2;
      align-self: start;
    }
