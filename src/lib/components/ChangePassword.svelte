...
<Modal title={success ? 'Password berhasil diubah' : 'Ganti password'} onclose={close}>
  {#if success}
    <div class="password-success" role="status">
    ...
    <button class="button submit" onclick={close}
      >Masuk kembali<Icon name="arrow" size={17} /></button
    >
  {:else}
    <p class="intro">
    ...
    >
      ...
      <ul id="change-rules">
        {#each rules as rule}<li class:met={rule.met}>
            <Icon name={rule.met ? 'check' : 'clock'} size={15} />{rule.label}<span class="sr-only"
              >: {rule.met ? 'Terpenuhi' : 'Belum terpenuhi'}</span
            >
          </li>{/each}
      </ul>
      {#if password && password === currentPassword}<p class="error">
      ...
      <p id="change-match" class:met={matches} class="match" aria-live="polite">
        {confirmation
          ? matches
      ...
      <div class="actions">
        <button class="button secondary" type="button" disabled={busy} onclick={close}>Batal</button
        ><button class="button" type="submit" disabled={busy || !valid}
