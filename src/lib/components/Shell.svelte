...
<div class="app-main">
  ...
  <main id="main-content" class="content"><section class="backend-notice" aria-label="Status sumber data"><div><strong>PocketBase lokal · Data seed · Hanya baca</strong><small>{app.session?.name} · {app.loadedAt ? 'Diperbarui ' + new Date(app.loadedAt).toLocaleTimeString('id-ID') : 'Belum dimuat'}{app.stale ? ' · Pembaruan gagal; data terakhir ditampilkan' : ''}</small><small>Penyimpanan menyusul P3.</small></div><button class="button secondary small" disabled={app.loading} onclick={() => app.reload()}>{app.loading ? 'Memuat…' : 'Muat ulang data'}</button></section>{@render children()}</main>
