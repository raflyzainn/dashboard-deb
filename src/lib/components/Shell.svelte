...
{#snippet navigation()}
  ...
  <div class="workspace">
    <span class="workspace-icon"
      ><Icon name={app.session?.role === 'admin' ? 'globe' : 'campus'} /></span
    >
    <div>
      <strong>{app.session?.role === 'admin' ? 'Pertamina Foundation' : campusProfile?.name}</strong
      ><small>{app.session?.role === 'admin' ? 'Ruang kerja Admin PF' : 'Ruang kerja kampus'}</small
      >
    </div>
  </div>
  ...
  <nav aria-label="Navigasi utama">
    {#each menus as key}
      {#if key === 'questions'}<div class="nav-divider"></div>
        <p class="nav-caption">BELAJAR BERSAMA</p>{/if}
      <a
        href={`${prefix}/${key}`}
  ...
  <div class="sidebar-bottom">
    ...
    <div class="sidebar-note">
      <Icon name="leaf" /><strong>Langkah kecil.<br />Dampak berkelanjutan.</strong>
      <p>Tumbuh bersama kampus dan masyarakat.</p>
    </div>
    <button
