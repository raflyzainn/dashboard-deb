...
{#if !compact}<div class="toolbar">
    ...
    <select aria-label="Filter kampus" bind:value={filter} onchange={() => (page = 1)}
    ...
    ><select aria-label="Filter wilayah kampus" bind:value={region} onchange={() => (page = 1)}
      ><option value="all">Semua wilayah</option>{#each CAMPUS_REGIONS as area}<option value={area}
          >{area}</option
        >{/each}</select
    ><span class="muted">{rows.length} kampus</span>
  </div>{/if}
{#if !visible.length}<Empty
  ...
  />{:else}<div class="table-scroll">
    <table>
      <thead
        ><tr
          ><th>KAMPUS MITRA</th><th>PROGRES DEB</th><th>PROPOSAL</th><th>STATUS</th><th
      ...
      ><tbody
        >{#each visible as row}<tr
            ><td
              ><a class="campus-name" href={`/admin/campuses/${row.id}`}
                ><span
                  class="table-avatar"
                  style:font-size={row.initials.length > 4 ? '9px' : undefined}>{row.initials}</span
                ><span
                ...
                ></a
              ></td
            ><td class="progress-cell"><Progress value={row.progress} showValue /></td><td
            ...
            ><td
              >{#if row.revisions}<Badge tone="amber">Perlu tindak lanjut</Badge>{:else}<span
                  class="muted">{row.progress >= 100 ? 'Tercapai' : 'Dalam proses'}</span
                >{/if}</td
            ><td
              ><a
                class="icon-button"
...
{#if !compact && rows.length}<div class="pagination">
    <span
    ...
    >
    <div>
      <button class="button secondary small" disabled={page <= 1} onclick={() => page--}
        >Sebelumnya</button
