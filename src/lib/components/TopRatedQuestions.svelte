...
<section class="top-rated" aria-labelledby="top-rated-title">
  <div class="top-rated-heading">
    <div>
      <span class="eyebrow">PILIHAN KOMUNITAS</span>
      <h2 id="top-rated-title">Top Rated Questions</h2>
      <p>Pertanyaan dengan like terbanyak di seluruh forum.</p>
    </div>
    <span class="top-rated-symbol"><Icon name="like" size={22} /></span>
  </div>
  {#if topQuestions.length}
    <ol class="top-rated-grid">
      {#each topQuestions as question, index}
        <li>
          ...
          >
            <div class="card-top">
              <span class="rank">#{index + 1}</span><span class="likes"
            ...
            <div class="card-bottom">
              ...
              ><span class="open-question">Lihat diskusi<Icon name="arrow" size={15} /></span>
            </div>
          </a>
        </li>
      {/each}
    </ol>
  {:else}<p class="top-rated-empty">
...
<style>
  ...
  .top-rated-card:hover {
    border-color: #9bb989;
    box-shadow: 0 4px 16px #294d2010;
  }
  .top-rated-card:focus-visible {
    outline: 2px solid #428750;
    outline-offset: 3px;
  }
  .card-top {
    display: flex;
  ...
  .top-rated-grid li:first-child .rank {
    background: #fff1ce;
    color: #926d1d;
  }
  .likes {
